...
import math
import h5py
from generator import convert_test_input
...
def extract_test_patches(img_data, img_label=None,normalise=True):
	...
	assert len(img_data.shape)==5, ' dimentional of volume image data must be 5..'
	...
	for i in xrange(patches_index.shape[0]):
		...
		for _ in xrange(batch_size):
			...
			# print ('>> crop from (%s,%s,%s) ' % (d_ran, h_ran, w_ran))
			...
			if img_label is not None:
				...
				# print '** gen d_ran:%d, h_ran:%d, w_ran:%d and(d,h,w): (%d,%d,%d)' % (d_ran,h_ran,w_ran,d,h,w)
