...
def find_data_path(s):
    r = re.compile(r'(?:myData\/)(.*?)(?:\/T1)',re.S)
    m = r.search(s)
    if m is None:
        return None
    else:
...
def find_data_split(t,s):
    r = re.compile(r'(\d)(?:,'+t+')',re.S)
    m = r.search(s)
    if m is None:
        return None
    else:
...
def record_ini(d,c):
    ...
    if len(df.loc[df['CONFIG_FILE-path']==p])>0:
        l = np.where(df['CONFIG_FILE-path']==p)[0][0]
    
    dfile = open(d+"T1.csv",'rb+')
    dstr = dfile.read().decode('utf-8')
    df.loc[l,'Data-path'] = find_data_path(dstr)
    
    sfile = open(d+"data_split",'rb+')
    sstr = sfile.read().decode('utf-8')
    
    df.loc[l,'Data-validation'] = find_data_split('validation',sstr)
    df.loc[l,'Data-inference'] = find_data_split('inference',sstr)
...
if __name__=="__main__":
    ...
    record_ini(args.model_dir,"model_record.csv")